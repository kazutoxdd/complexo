...
local itensRestritos = {
	...
	"lean",
	"identity-"
	-- Adicione os itens quais não e pra descartar
  }
  
RegisterServerEvent("inventory:destroyItem")
AddEventHandler("inventory:destroyItem", function(Slot, Amount)
		local source = source
		local Slot = tostring(Slot)
		local Amount = parseInt(Amount)
		local user_id = vRP.getUserId(source)
	  ...
	  -- Verificar se o item está na lista de itens restritos
	  local itemRestrito = false
	  for _, item in ipairs(itensRestritos) do
		if Item == item then
		  itemRestrito = true
		  break
		end
	  ...
	  if itemRestrito then
		TriggerClientEvent("Notify",source,"amarelo","Item Proibido.",5000)
		--print("Tentativa de descartar um item restrito: " .. Item)
		return
	  end
			...
			if vDELIVER.Deliver(source,"BurgerShot") then
				if vRP.tryGetInventoryItem(user_id,totalName,Amount,false,Slot) then
					...
					if nameItem == "burgershot3" then
						...
						local burgershotReputation = vRP.checkReputation(user_id,"Delivery")
						...
						if burgershotReputation <= 1001 then
							vRP.insertReputation(user_id,"Delivery",10)
						end
			...
			if vDELIVER.Deliver(source,"PizzaThis") then
				if vRP.tryGetInventoryItem(user_id,totalName,Amount,false,Slot) then
					...
					if nameItem == "pizzathis3" then
						...
						local pizzathisReputation = vRP.checkReputation(user_id,"Delivery")
			...
			if vDELIVER.Deliver(source,"UwuCoffee") then
				if vRP.tryGetInventoryItem(user_id,totalName,Amount,false,Slot) then
					...
					if nameItem == "uwucoffee3" then
						...
						local uwucoffeeReputation = vRP.checkReputation(user_id,"Delivery")
			...
			if vDELIVER.Deliver(source,"BeanMachine") then
				if vRP.tryGetInventoryItem(user_id,totalName,Amount,false,Slot) then
					...
					if nameItem == "beanmachine3" then
						...
						local beanmachineReputation = vRP.checkReputation(user_id,"Delivery")
	...
	if user_id and Active[user_id] == nil then
		...
		if itemType(totalName) == "Armamento" and parseInt(Slot) <= 5 then
			...
			if returnWeapon then
				...
				if Attachs[user_id][nameItem] == nil then
		...
		if nameItem == "attachsFlashlight" or nameItem == "attachsCrosshair" or nameItem == "attachsSilencer" or nameItem == "attachsGrip" or nameItem == "attachsMagazine" or nameItem == "attachsMuzzleHeavy" or nameItem == "attachsBarrel" or nameItem == "attachsMuzzleFat" then
			local returnWeapon = vCLIENT.returnWeapon(source)
			if returnWeapon then
				if Attachs[user_id] == nil then
					Attachs[user_id] = {}
		...
		if itemType(totalName) == "Usável" or itemType(totalName) == "Animal" then
			...
			if nameItem == "dessertspass" then
				...
				if distance <= 5 then
					...
					if parseInt(#FoodsResult) >= 1 then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							vRP.generateItem(user_id,"nigirizushi",3,true)
							vRP.generateItem(user_id,"sushi",3,true)
							vRP.generateItem(user_id,"dollars",200,true)
							TriggerClientEvent("inventory:Update",source,"updateMochila")
						end
					end
				end
			...
			if nameItem == "burgershotpass" then
				...
				if distance <= 5 then
					...
					if parseInt(#FoodsResult) >= 1 then
						...
						if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
							vRP.generateItem(user_id,"hamburger2",1,true)
							vRP.generateItem(user_id,"cookedmeat",2,true)
				...
				if not vPLAYER.getHandcuff(source) then
					...
					if homeName then
						...
						if (Proper[user_id] == nil or os.time() > Proper[user_id]) then
							...
							TriggerClientEvent("propertys:togglePropertys",source)
			...
			if nameItem == "sprays01" then
				TriggerClientEvent("inventory:Close",source)

				TriggerClientEvent("inventory:Buttons",source,true)
				local application,coords,heading = vRPC.objectCoords(source,"spray_01")
				if application then
					vRPC.stopActived(source)
					TriggerClientEvent("inventory:Close",source)
					TriggerClientEvent("inventory:Buttons",source,true)
					...
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
						...
						Objects[tostring(Number)] = { x = mathLegth(coords["x"]), y = mathLegth(coords["y"]), z = mathLegth(coords["z"]), h = heading, object = "spray_01", item = totalName, distance = 100, mode = "Spray" }
			...
			if nameItem == "suitcase" then
				if vRP.tryGetInventoryItem(user_id,totalName,1,true,Slot) then
					vRP.generateItem(user_id,"suitcase",1,false)
					vRP.generateItem(user_id,"dollars",splitName[2],false)
					TriggerClientEvent("inventory:Update",source,"updateMochila")
				end
			return end
			...
			if nameItem == "pizzamozzarella" or nameItem == "pizzamushroom" or nameItem == "pizzabanana" or nameItem == "pizzachocolate" then
				vRPC.stopActived(source)
				Active[user_id] = os.time() + 10
				TriggerClientEvent("Progress",source,10000)
				TriggerClientEvent("inventory:Close",source)
			...
			if nameItem == "premiumplate" then
				if vRPC.inVehicle(source) then
					...
					if vehicle[1] then
						...
						if not vehPlate or string.upper(vehPlate[1]) == "COMPLEXO" then
							return
						end
...
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("inventory:saveTemporary",function(user_id)
	if Temporary[user_id] == nil and Ammos[user_id] and Attachs[user_id] then
		Temporary[user_id] = {
			["Ammos"] = Ammos[user_id],
			["Attachs"] = Attachs[user_id]
		}
...
-----------------------------------------------------------------------------------------------------------------------------------------
AddEventHandler("inventory:applyTemporary",function(user_id)
	if Temporary[user_id] and Ammos[user_id] and Attachs[user_id] then
		Attachs[user_id] = Temporary[user_id]["Attachs"]
		Ammos[user_id] = Temporary[user_id]["Ammos"]
		Temporary[user_id] = nil
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PLAYERDISCONNECT
...
AddEventHandler("playerDisconnect",function(user_id)
	if Ammos[user_id] and Attachs[user_id] then
		if Temporary[user_id] then
			Ammos[user_id] = Temporary[user_id]["Ammos"]
			Attachs[user_id] = Temporary[user_id]["Attachs"]
			Temporary[user_id] = nil
		end
